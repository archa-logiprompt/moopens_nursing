...
class Payment extends Parent_Controller {
    ...
    public function pay($student_fees_master_id, $fee_groups_feetype_id, $student_id, $fine = 0)
    {
        ...
        if (!empty($this->payment_method)) {
            ...
            if ($pay_method->payment_type == "paypal") {
                ...
                $this->session->set_userdata("params", $params);
                redirect(base_url("parent/ccavenue"));
            } else if ($pay_method->payment_type == "nttdata") {
                ...
                $this->session->set_userdata("params", $params);
                redirect(base_url("parent/nttdata"));
                //=======================
            } else {
                $this->session->set_flashdata('error', 'Something wrong');
                redirect($_SERVER['HTTP_REFERER']);
    ...
    public function paybulk($student_fees_master_id, $fee_groups_feetype_id, $student_id, $fine = 0, $bulk_fine, $total_amount, $bulk_amount)
    {
        // $student_fees_master_id_array = explode("-", $student_fees_master_id);
        // $fee_groups_feetype_id_array = explode("-", $fee_groups_feetype_id);
        $this->session->unset_userdata("params");
        ///=======================get balance fees
        ...
        if (!empty($this->payment_method)) {
            ...
            if ($pay_method->payment_type == "paypal") {
                ...
                foreach (explode('-', $fee_groups_feetype_id) as $fee_id) {

                    $payment_detail = $this->feegrouptype_model->getFeeGroupByID($fee_id);
                ...
                $this->session->set_userdata("params", $params);
                redirect(base_url("parent/nttdata/nttdataPayment"));
                //=======================
            } else {
                $this->session->set_flashdata('error', 'Something wrong');
                redirect($_SERVER['HTTP_REFERER']);
