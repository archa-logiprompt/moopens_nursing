...
class Monthlyacademicreport extends Admin_Controller
{
    ...
    public function monthlyattendancereport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $theory_pm =
                $this->db->select('students.firstname,students.lastname,COUNT(student_period_attendance.id) as total,
SUM(student_period_attendance.attendencetype = 0) as present_count')->from('student_session')
                ...
                ->where(['student_period_attendance.class_id' => $data['class_id'], 'student_period_attendance.section_id' => $data['section_id'], 'session_id' => $session,'subject_id'=>$subject_id])->where("STR_TO_DATE(date, '%d/%m/%Y') < '$start'")->order_by('students.firstname')->group_by('students.id')
                ->get()->result_array();
    ...
    SUM(student_period_attendance.attendencetype = 0) as present_count')->from('student_session')
            ...
            SUM(clinical_attendance.attendence_type_id = 1) as present_count')
                ...
                ->where([
                    ...
                    'clinical_attendance.session_id' => $session,'subject_id'=>$subject_id
                ])
