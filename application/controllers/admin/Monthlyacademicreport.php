...
class Monthlyacademicreport extends Admin_Controller
{
    ...
    public function monthlyattendancereport()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['month_name'] = date('F', mktime(0, 0, 0, $month, 1));
            $data['year'] = $year;
            $acadamicstart = $this->db->select('from')->where(['class_id' => $class_id, 'section_id' => $section_id])->get('academic_date')->row()->from;
            $start = date("Y-m-01", strtotime("$year-$month-01"));
            $acadamicstart = date("Y-m-d", strtotime("$acadamicstart"));
            ...
            $total = $this->db->select('actual')
                ->from('student_period_attendance')
                ->where([
                    'student_period_attendance.class_id' => $data['class_id'],
                    'student_period_attendance.section_id' => $data['section_id'],
                    'subject_id' => $subject_id
                ])
                ->where("STR_TO_DATE(date, '%d/%m/%Y') BETWEEN '$acadamicstart' AND '$end'")
            ...
            $totalActual = array_column($total, 'actual');
            $totalActual_cm = array_column($total_cm, 'actual');




            // Calculate the sum of the 'actual' values
            $sum = array_sum($totalActual);
            $sum_cm = array_sum($totalActual_cm);
            // var_dump($sum/60);exit;
            $data['total'] = $sum;
            $data['total_cm'] = $sum_cm;




            $theory_pm =
                $this->db->select('students.firstname,students.lastname,sum(minutes) as minutes')->from('student_session')
                ->join('students', 'student_session.student_id=students.id', 'left')
                ->join('student_period_attendance', 'student_period_attendance.student_id = student_session.student_id', 'left')
                ->where(['student_period_attendance.class_id' => $data['class_id'], 'student_period_attendance.section_id' => $data['section_id'], 'session_id' => $session, 'subject_id' => $subject_id])->where("STR_TO_DATE(date, '%d/%m/%Y') BETWEEN '$acadamicstart' AND '$end'")->order_by('students.firstname')->group_by('students.id')
                ->get()->result_array();
...
//  echo $this->db->last_query();exit;
            ...
            $theory_cm =
                ...
                ->join('students', 'student_session.student_id=students.id', 'left')
                ->join('student_period_attendance', 'student_period_attendance.student_id = student_session.student_id', 'left')
                ->where(['student_period_attendance.class_id' => $data['class_id'], 'student_period_attendance.section_id' => $data['section_id'], 'session_id' => $session, 'subject_id' => $subject_id])->where("STR_TO_DATE(date, '%d/%m/%Y') BETWEEN '$start' AND '$end'")->order_by('students.firstname')->group_by('students.id')
                ->get()->result_array();
            ...
            $practical_cm =
                $this->db->select('students.firstname, students.lastname, COUNT(clinical_attendance.id) as total, 
            SUM(clinical_attendance.attendence_type_id = 1) as present_count')
                ->from('student_session')
                ->join('students', 'student_session.student_id = students.id', 'left')
                ->join('clinical_attendance', 'clinical_attendance.student_session_id = student_session.id', 'left')
                ->where([
                    'class_id' => $data['class_id'],
                    'section_id' => $data['section_id'],
                    'clinical_attendance.session_id' => $session,
                    'subject_id' => $subject_id
                ])
            ...
            $student = array();
            $student['theory_pm'] = $theory_pm;
            $student['theory_cm'] = $theory_cm;
            $student['practical_pm'] = $practical_pm;
            $student['practical_cm'] = $practical_cm;

            $data['student'] = $student;

            $data['students']['theory'] = $$wherearray = [
                'class_id' => $class_id,
                'section_id' => $section_id,
            ];
            $this->load->view('layout/header', $data);
