...
class Attendencebyday_model extends CI_Model {
    ...
    if (isset($data['id'])) {
        ...
        $logData = array(
            ...
            // 'status' => 'Updated attendance for student_session_id: ' . $data['student_session_id'],
        ...
        $logData = array(
            'user_id' =>  $data['user_id'],// Assuming user_id is stored in session
            // 'status' => 'Added new attendance for student_session_id: ' . $data['student_session_id'],
             'status'=>' Attendance Inserted',
            'created_at' => date('Y-m-d H:i:s')
        );
