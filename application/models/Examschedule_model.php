...
class Examschedule_model extends CI_Model {
    ...
    // }
    public function getDetailbyClsandSection($class_id, $section_id, $exam_id, $teacher_id = null)
    {
        $admin = $this->session->userdata('admin');
        $centre_id = $admin['centre_id'];
$query = $this->db->query("SELECT exam_schedules.*, teacher_subjects.teacher_id, subjects.name, subjects.id as subject_id, subjects.type 
    FROM exam_schedules, teacher_subjects, exams, class_sections, subjects 
    WHERE exam_schedules.teacher_subject_id = teacher_subjects.id 
    AND exam_schedules.exam_id = exams.id 
    AND class_sections.id = teacher_subjects.class_section_id 
    AND teacher_subjects.subject_id = subjects.id 
    ...
    AND exam_schedules.centre_id = " . $this->db->escape($centre_id) . " 
    AND exam_schedules.session_id = " . $this->db->escape($this->current_session) . " 
    ORDER BY exam_schedules.date_of_exam ASC");
...
// echo $this->db->last_query();
     ...
     public function getExamByClassandSection($class_id, $section_id) {
        ...
        GROUP BY exams.id 
        ORDER BY exam_schedules.date_of_exam"; // Add this line for sorting by exam name
